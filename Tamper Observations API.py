...
# Set the time range for the previous day and break it into 6-hour chunks
all_observations = []
end_date = datetime.now().replace(hour=0, minute=0, second=0, microsecond=0)
start_date = end_date - timedelta(days=1)

# Loop through each 6-hour period of the previous day
for i in range(0, 24, 6):
    period_start = start_date + timedelta(hours=i)
    period_end = period_start + timedelta(hours=6)
    query = f"device_timestamp:[{period_start.strftime('%Y-%m-%dT%H:%M:%SZ')} TO {period_end.strftime('%Y-%m-%dT%H:%M:%SZ')}] AND (observation_type:TAMPER)"
...
# Save to Excel
# observations_df.to_excel("C:/path/to/your/directory/Tamper_Observations.xlsx", index=False)
...
if not observations_df.empty:
    load_to_sql(observations_df, "Tamper_Observations_Data")
